...
class Customer extends BaseController{
    ...
    public function overview()
    {
        $repo = new CustomerRepo;
        $view = new View();
        $view ->CustomerOverview($repo->allInfo(),$this->filter());
    }
    ...
    public function filter(): ?array{
      ...
      if($this->request->getRequestMethod() === 'POST'){
            if( $this->request->verifyIfIsset('search')!=null || $this->request->verifyIfIsset('reset')!=null ){
               ...
               $repo = new CustomerRepo;   
               $sqlFilter = 'select id, name, adress from customer';
               $search= $this->request->getValue('search_box');
               $sqlFilter  .=" where name LIKE  '%{$search}%'";
               $sqlFilter  .=" or adress LIKE  '%{$search}%'";
    ...
    public function serviceInsurance($idInsurance){
    ...
    public function price($idInsurance,$type){
      $repoServices = new Services;
      $services = $repoServices->getServicesDetails($idInsurance->id_insurances);
      $price = 0;
      foreach ($services as $service) {
    ...
    public function detailsInsurances($idCustomer){
